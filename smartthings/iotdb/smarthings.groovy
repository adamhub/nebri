...
private _device_command(device, type, jsond) {
   ...
   */
   ...
   def arguments = jsond['arguments']
   
   if (command) {
       if (arguments) {
           device."$command"(*arguments)
           log.debug "Command triggered: ${command} with the arguments: ${arguments}"
       } else {
           device."$command"()
           log.debug "Command triggered: ${command} with NO ARGUMENTS"
       }
   } else {
       log.debug "Missing command argument"
   }
}
...
private _device_to_json(device, type) {
    ...
    def vd = [:]
    def jd = [id: device.id, name: device.name, label: device.label, type: type, value: vd];
